    ...
    <div>
        <div style="height: 30px;line-height: 30px;margin-bottom: 15px;padding:5px;background:#fff">
            <el-breadcrumb separator="/" style="line-height:30px">
                 <el-breadcrumb-item>已验收订单列表</el-breadcrumb-item>
  ...
  <el-table :data="tableData" style="width: 100%">
    ...
    <el-table-column label="操作" width="300">
      <template slot-scope="scope">
        <el-link type="primary"  @click="open_Detail(scope.row)">详情</el-link>  
        <el-link type="primary"  @click="open_BS(scope.row)">标识</el-link>
        <el-link type="primary"  @click="handleFp_mess(scope.$index, scope.row)">开发票时间</el-link>
        <el-link type="primary"  @click="handleFp_time_Received(scope.$index, scope.row)">收发票时间</el-link>
        <el-link type="primary"  @click="open_Dialog(scope.row)">收款信息</el-link>
      </template>
    </el-table-column>
  </el-table>
...
<script>
  export default {
    ...
    methods: {
      ...
      paging(){
          let postData = this.$qs.stringify({'order_zt':5,'pageSize':this.pageSize,'currentPage':this.currentPage});
          console.log(postData)
